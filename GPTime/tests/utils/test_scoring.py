...
class TestMetrics(unittest.TestCase):
    ...
    def test_predictM4(self):
        ...
        # shape
        self.assertEqual(naive_predictions.shape, (100000, 48), "Should be (100000, 48)")
    ...
    def test_scoreM4(self):
        ...
        naive_predictions = predict_M4(model=self.naive)
        mlp_predictions = predict_M4(model=self.mlp)
        naive_scores = score_M4(naive_predictions, f"GPTime/tests/results/M4/naive_test{rand_int}.csv")
        mlp_scores = score_M4(mlp_predictions, f"GPTime/tests/results/M4/mlp_test{rand_int}.csv")

        # scores were saved
        df_naive = pd.read_csv(f"GPTime/tests/results/M4/naive_test{rand_int}.csv")
